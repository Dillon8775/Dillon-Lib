...
/**
 * A class for creating a a list of options.
 */
@GlobalUse
public abstract class BaseOptions<T> {
    public static final String OPTIONS_ERROR_MESSAGE = "Found error with settings, launching in safe mode.";
    public static final String OPTIONS_WARNING_MESSAGE = "Found an unusual value in settings.";
    ...
    public void setBroken(OptionValue<?> option, String value) {
        DillonLib.error(OPTIONS_ERROR_MESSAGE + related + "options."+value);
        DillonLib.isSafe(false);
        option.setBroken();
    }
    ...
    public void throwNullPointerException(String option, Object[] values) {
        throw new NullPointerException("Option \""+option+"\" is NULL. Most likely, it is set to an invalid value in the options .json file. You can either delete the config file to automatically re-generate it correctly upon next launch, or set it to a valid value. Valid values for \""+option+"\" are: "+ Arrays.toString(values));
    }
