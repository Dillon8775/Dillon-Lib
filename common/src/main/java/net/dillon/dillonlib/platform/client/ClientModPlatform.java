...
public abstract class ClientModPlatform implements Loadable {
     ...
     */
    public abstract KeyMapping registerKeyMapping(String name, InputConstants.Type type, KeyMapping.Category category, int value);
    ...
    /**
     * Registers any event that should be called when a {@link LocalPlayer} joins a world/server.
     */
    public void registerClientJoinEvents(Minecraft minecraft, PacketListener clientPacketListener) {
    }

    /**
     * Registers any event that should be called when a {@link LocalPlayer} disconnects from a world/server.
     */
    public void registerClientDisconnectEvents(Minecraft minecraft) {
    }
